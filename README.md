...
- If neither flag nor environment variable is provided, the default is `classic`.
...
- If `cargo install gh-bofh` fails: ensure you have a compatible Rust toolchain installed (see project MSRV in the [`builds`](#builds) matrix). You can use `rustup` to install the required toolchain.
